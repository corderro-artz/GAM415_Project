...
void AProcMeshFromStatic::GetMeshData()
{
	...
	if (mesh)
	{
		...
		procMesh->UpdateMeshSection(0, Vertices, Normals, UV0, UpVertexColor, Tangents);
		CreateMesh();
	}
}
...
void AProcMeshFromStatic::CreateMesh()
{
	if (baseMesh)
	{
		procMesh->CreateMeshSection(0, Vertices, Triangles, Normals, UV0, UpVertexColor, Tangents, true);
	}
}
